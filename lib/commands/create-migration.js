...
// patriq => this function actually creates the migrations themselves.
//            the code thats auto-inserted is just pulled from templates
//            which are only strings...
async function executeCreateMigration (internals, config) {
  ...
  // patriq => this function call is what actually returns/makes the base SQL files
...
async function createSqlFiles (internals, config) {
  ...
  // patriq => If we want to modify/remove down migrations, this is a good place to start!!
  const migrationDownSQL = new Migration(
    internals.argv.title + '-down.sql',
    sqlDir,
    internals.runTimestamp,
    '/* We dont actually really support down migrations! */',
  );
