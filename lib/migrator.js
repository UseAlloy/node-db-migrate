...
Migrator.prototype = {
  ...
  writeMigrationRecord: function (migration, callback) {
    function onComplete (err, notransactions) {
      if (err) {
        log.error(migration.name, err);
      } else {
        const migrationMessage = `Processed migrations ${notransactions ? 'without' : 'with'} transactions`;
        log.info(migrationMessage, migration.name);
      }
      callback(err);
    }
    this._driver.addMigrationRecord(
      this.internals.matching + '/' + migration.name,
      (err) => onComplete(err, this.internals.notransactions)
    );
  },
    ...
    this._driver.deleteMigration(
      ...
      function (err) {
        if (!this.internals.matching) {
          this._driver.deleteMigration(migration.name, (err) => onComplete(err, this.internals.notransactions));
        } else {
